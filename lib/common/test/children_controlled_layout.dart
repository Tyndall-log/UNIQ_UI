...
class ChildrenControlledLayout extends MultiChildRenderObjectWidget {
  const ChildrenControlledLayout({
    super.key,
...
class ChildrenControlledLayoutDelegate<T>
  ...
  final double height;
  ChildrenControlledLayoutDelegate(this.children, {required this.height});

  /// ** instead of returning void, it now returns the accumulated size**
  @override
  Size performLayout(Size size) {
    ...
    for (var child in children) {
      if (hasChild(ValueKey(child))) {
        final size = layoutChild(ValueKey(child), constraints);
        positionChild(ValueKey(child), offset);
...
abstract class CustomMultiChildLayoutDelegate {
  ...
  ///** Edited this function to return the size of the child it has laid out**
...
class RenderCustomMultiChildLayoutBox extends RenderBox
  ...
  void performLayout() {
    ...
    size = delegate._callPerformLayout(_getSize(constraints), firstChild);
  }
